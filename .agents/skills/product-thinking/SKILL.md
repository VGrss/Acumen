...
4. **Check `.acumen/` directory** for extended context files — competitor briefs, persona research, feature inventories, interview notes. Load what is relevant to the current task, not everything. **Check staleness**: each file carries a `_Last updated: [date]_` line. If any file relevant to the current task is older than 90 days, warn the user before proceeding: "`.acumen/[file]` was last updated [date] — consider re-running `/[command]` to refresh it." Do not silently build analysis on stale context.
